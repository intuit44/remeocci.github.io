...
class BotWhatsAppPlayMall {
    ...
    async procesarVideoRecibido(message) {
        try {
            ...
            if (this.videoAnalyzer.constructor.name === 'VideoAnalyzerPremium') {
                ...
                return await this.videoAnalyzer.procesarVideoCompleto(message);
            }
    ...
    // CORRECCIÓN ADICIONAL: Función procesarVideoBasico más robusta
    async procesarVideoBasico(message) {
        ...
        try {
            ...
            try {
                ...
                videoInfo.isGroup = true; // Asumir grupo por defecto
            }
            ...
            const resultado = {
                ...
                mensaje: `✅ Video recibido del parque\n📊 ID: ${videoInfo.id.slice(-8)}\n⚙️ Procesamiento: Básico completado`,
                videoPath: null,
            ...
            return {
                success: false,
                error: error.message,
                mensaje: `❌ Error en procesamiento básico: ${error.message}`
            };
        }
    }
    ...
    async enviarRespuestaSegura(message, textoRespuesta) {
        try {
            ...
            // === VALIDACIÓN Y LIMPIEZA DEL TEXTO ===
            ...
            // LIMPIAR CARACTERES PROBLEMÁTICOS QUE CAUSAN EL ERROR
            ...
            // LÍMITE ESTRICTO
            const LIMITE = 4000;
            if (textoRespuesta.length > LIMITE) {
                textoRespuesta = textoRespuesta.slice(0, LIMITE - 100) + '\n\n... (mensaje truncado)';
            }

            console.log(`[RESPONSE] 🟢 Preparando: ${textoRespuesta.length} chars para ${chatId}`);
            ...
            // === MÉTODO 1: getChatById (MÁS ESTABLE) ===
            try {
                console.log('[RESPONSE] 🔄 Método 1: getChatById...');
                ...
                // === MÉTODO 2: client.sendMessage DIRECTO ===
                try {
                    console.log('[RESPONSE] 🔄 Método 2: client.sendMessage...');
...
`;
    ...
    async procesarVideoConSistemaCompleto(message) {
        try {
            ...
            console.log('[VIDEO] ✅ Análisis premium de video completado');
            return {
                tipo: 'video_premium',
    ...
    generarReporteVideoCompleto(datos) {
        ...
        if (analisisTecnico) {
            ...
            reporte += `${this.formatearAnalisisTecnico(analisisTecnico)}\n\n`;
            reporte += `${'─'.repeat(60)}\n\n`;
        }
            ...
            if (esImagen) {
                ...
                try {
                    ...
                    if (videoData && videoData.status === 'success') {
                        ...
                        if (this.instagramProcessor) {
                            mensajeInstagram = await this.instagramProcessor.procesarVideoParaInstagram(
                                videoData, 
                                message, 
                                analisisIA
                            );
                        ...
                        const respuestaCompleta = await this.combinarAnalisisVideoInstagram(
                            videoData, 
                            analisisIA, 
                            mensajeInstagram
                        );
                        ...
                        await message.reply(respuestaCompleta);
                        console.log('[VIDEO] Respuesta completa enviada');
                        return;
    ...
    async combinarAnalisisVideoInstagram(videoData, analisisIA, mensajeInstagram) {
        try {
            ...
            if (analisisIA && analisisIA.respuesta_grupo && analisisIA.respuesta_grupo.length > 0) {
                ...
                if (analisisIA.respuesta_grupo.includes('INTELIGENCIA EMPRESARIAL')) {
                    const lineasIA = analisisIA.respuesta_grupo.split('\n');
                    const resumenIA = lineasIA.slice(0, 10).join('\n');
                    respuestaCompleta += `${resumenIA}\n\n`;
                    respuestaCompleta += `   📋 *[Análisis completo disponible en logs del sistema]*\n\n`;
                } else {
                    respuestaCompleta += `${analisisIA.respuesta_grupo}\n\n`;
                }
            }
            ...
            respuestaCompleta += `📹 Video + 🧠 Google Vision + 📱 Instagram = 🚀 Automatización completa`;
            
            return respuestaCompleta;
            
        } catch (error) {
            console.error('[ERROR] Error combinando análisis:', error);
            return mensajeInstagram || `🎬 **Video procesado correctamente**\n\nVideo documentado en el sistema para revisión posterior.`;
        }
    }
